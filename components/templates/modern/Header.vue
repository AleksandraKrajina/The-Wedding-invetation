<template>
  ...
  >
    ...
    <div class="p-6 border-t mb-6 w-full hover:bg-gray-700 text-center transition-colors">
      <button
        class="block w-full text-2xl p-2 rounded-lg"
        @click="openOverlay('rsvp')"
      >
        Accept
      </button>
    </div>
  ...
  <transition name="fade">
    <div
      v-if="isOverlayOpen"
      class="fixed inset-0 z-50 flex flex-col items-center justify-center pt-32 bg-black"
    >
      <button
        class="absolute border p-4 top-14 left-14 md:left-24 hover:text-3xl text-white text-2xl "
        @click="closeOverlay"
      >
        Close
      </button>
